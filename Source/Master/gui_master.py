...
class MasterGUI(QWidget):
    ...
    def __init__(self):
        ...
        # Onglets
        tabs = QTabWidget()
        tabs.addTab(self.moniteur(), "Accueil")
    ...
    #ajout des chemins dans l'onglet accueil
    ...
    def ajouter_routeur_client(self):
        ...
        # Liste déroulante pour le choix des ajouts des équipements
    ...
    # Actualisation et remplit la liste déroulante pour le choix des next hop dans l'onglet Topologie réseau
    ...
    # Permet la création et le lancement du cmd pour l'onglet "AJOUTER UN ROUTEUR OU UN CLIENT"
    ...
    #Pour refresh pour le gui les routeurs et les clients avec le bouton "Actualiser" dans l'onglet accueil / est utiliser juste en dessous avec refresh all
    ...
    #
    # Lancement du terminal pour le lancement des équipements selon l'os
    #
    def lancer_terminal(self, directory, type_objet, nom, port):
        if sys.platform.startswith("win"):
            # WINDOWS
            if type_objet == "routeur":
                cmd = f'cmd /k "cd /d {directory} && python router.py {nom} {port}"'
            else:
                cmd = f'cmd /k "cd /d {directory} && python client.py {nom} {port}"'

        else:
            # LINUX
            if type_objet == "routeur":
                cmd = f'xterm -hold -e "cd {directory} && python3 router.py {nom} {port}"'
            else:
                cmd = f'xterm -hold -e "cd {directory} && python3 client.py {nom} {port}"'
    ...
    #
    # Permet le refresh de l'onglet topologie réseau / est utiliser dans refresh all
    #
    ...
    #
    # Vérification des données next hop + envoie dans la DB pour l'info next hop
    #
